...
class BayesianEstimator:
    ...
    def run(self, signal, ref, d_init_um, n_init,
        ...
        band  = (f_THz > 0.3) & (f_THz < 3.0)
        f_hz  = f_THz[band] * 1e12

        S_f   = fft(signal)
        E_f   = fft(ref[:N])
        H     = S_f[band] / (E_f[band] + 1e-20)
        phi_m = np.unwrap(np.angle(H))

        hf    = f_THz > 3.5
        sig_n = float(np.std(np.abs(S_f[hf]) / (np.abs(E_f[hf]) + 1e-20))) + 0.05
        ...
        def ll(d, n):
            phi_p = -4*np.pi * f_hz * n * d / C_LIGHT
            diff  = np.angle(np.exp(1j*(phi_m - phi_p)))
            return -0.5 * np.sum(diff**2) / (sig_n**2 + 1e-30)

        def lp(d, n):
            if d <= 0 or n < 1.0 or n > 3.0: return -np.inf
            return -0.5*((d-d0)/sd)**2 - 0.5*((n-n_init)/0.06)**2
        ...
        if verbose:
            print(f"    [Bayesian MCMC] {total} iterations...", end="", flush=True)
